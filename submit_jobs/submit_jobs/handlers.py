...
def detailed_display(job):
    ...
    result = '<b>JobID</b>: {}<br><b>Status</b>: {}<br><b>Algorithm</b>: {}<br><b>Inputs</b>:<br>'.format(job_id, status, algo_id)
    for i in inputs:
        result += '&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<b>{}</b>: {}<br>'.format(i['name'], i['value'])
    return result
            ...
            if r.status_code == 200:
                try:
                    ...
                    if 'Exception' in r.text:
                        ...
                        self.finish({"status_code": 404, "results": result})
                    else:
                        job_id = rt[0].text
                        ...
                        # print("success!")
                        self.finish({"status_code": r.status_code, "results": result})
                except:
                    self.finish({"status_code": r.status_code, "results": r.text})
            # if no job id provided
            elif r.status_code in [404]:
                # print('404?')
                # if bad job id, show provided parameters
                ...
                try:
                    ...
                    self.finish({"status_code": r.status_code, "results": r.reason})
        except:
            self.finish({"status_code": 400, "results": "Bad Request"})
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                try:
                    ...
                    result += '<div id = "job-table" style="overflow:auto; max-height:45%; width: 435px; font-size:11px;">'
                    result += table
                    result += '</div>'
                    result += '</div>'
                    logging.debug(table)
                    ...
                    # print("success!")
                    self.finish({"status_code": r.status_code, "results": result, "table": table, "jobs": jobs_dict, "displays": details})
                except:
                    table = '<br>'.join(jobs)
                    self.finish({"status_code": r.status_code, "results": table, "table": table, "jobs": jobs, "displays": details, "resp": r.text})
            # if no job id provided
            elif r.status_code in [404]:
                # print('404?')
                # if bad job id, show provided parameters
                ...
                for f in fields:
                    table += '\t{}: {}\n'.format(f,params[f])
                table += '\n'
                self.finish({"status_code": 404, "results": table})
            else:
                self.finish({"status_code": r.status_code, "results": r.reason})
        except:
            self.finish({"status_code": 400, "results": "Bad Request"})
