    ...
    def get(self):
        ...
        try:
            ...
            # if status !- 0, user workspace not already mounted
                ...
                # logging.debug('{} created'.format(user_bucket_dir))
                ...
                logging.debug(touch_output)

                # # unmount bucket and mount user's subfolder
                # mountdir_output = subprocess.check_output('s3fs -o use_cache=/tmp/cache maap-mount-dev /projects/{}'.format(username), shell=True).decode('utf-8')
                # message = mountdir_output
                # logging.debug(mountdir_output)
